...
class MeshNeighborhoodLoss:
    ...
    def __init__(self,
        ...
        # ====== ここから追加  ======
        child_start = 6890                       # 親は 0〜6889，子は 6890〜
        num_child   = verts_init.shape[0] - child_start   # = F (= faces 行数)
    
        # faces の並びと同じ順で「face ごとの子インデックス」を作る
        child_idx = np.arange(child_start,
                              child_start + num_child,
                              dtype=np.int64)                  # (F,)
    
        # 子→親３本の“有向”エッジ
        cp_src = np.repeat(child_idx, 3)                       # (3F,)
        cp_dst = faces.reshape(-1)                             # (3F,)
        child_edges_np = np.stack([cp_src, cp_dst], axis=1)    # (3F,2)
    
        # 逆向き（親→子）も作って「無向」に
        child_edges_np = np.vstack([child_edges_np,
                                    child_edges_np[:, ::-1]])  # (6F,2)
    
        # 親↔親 と連結
        edges_np = np.vstack([edges_np, child_edges_np])       # (E + 6F,2)
        # ====== ここまで追加  ======

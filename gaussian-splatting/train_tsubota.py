...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        # 中心化
        means3D = means3D - means3D.mean(dim=0)
        # スケーリング（各点の原点からの距離を求めて最大距離で正規化）
        scale = means3D.norm(dim=1).max()
        ...
        loss2 = loss2.mean()
        loss3 = F.mse_loss(output, input, reduction='mean')
        eps = 1e-6
        lambda_opacity = 0.01
        opacity_penalty = torch.mean(1.0 / (opacity + eps))  # epsは0除算防止
        loss4 = lambda_opacity * opacity_penalty
        # loss = (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * (1.0 - ssim(image, gt_image))
        loss = (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * (1.0 - ssim(image, gt_image)) + loss2 + loss3 + loss4
        loss.backward()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[100, 1000, 3000, 7000, 30_000])
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[100, 1000, 3000, 7000, 30_000])

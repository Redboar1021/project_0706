...
def freeze_gaussians_var(model: GaussianModel, freeze: bool = True): 
    """freeze=True で Gaussians を凍結（勾配+更新を停止）"""
    # ① requires_grad を切り替え
    for tensor in (
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    vae.load_state_dict(torch.load("/root/development/project4010/vae_pointnet_vposer.pth",
                                   map_location=device))
    vae.eval()                      # ① 推論モード
    ...
    # pre_affine.log_s.data = torch.tensor([-3.912], device=device)  # 初期スケール

    opt_rot  = torch.optim.Adam(rot_params,  lr=5e-2)
    opt_main = torch.optim.Adam(main_params, lr=1e-3)
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if iteration < 1:
            ...
            if iteration == 0:
                ...
                if pca:
                    freeze_gaussians_var(gaussians, freeze=True)
        ...
        if pca:
            with torch.no_grad():
                gaussians._rotation.data.copy_(quat)
        ...
        loss = (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * (1.0 - ssim(image, gt_image)) + loss2 + loss3  +loss4 + li_white * 0.1
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[1, 100, 1000, 3000, 7000, 10000, 15000, 20000, 30000])
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[1 ,100, 1000, 3000, 7000, 10000, 15000, 20000, 30000])

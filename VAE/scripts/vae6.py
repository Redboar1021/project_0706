...
# ============================================================
#  util : SMPLify-X の .pkl を可視化（32-dim / 63-dim 両対応）
# ============================================================
def show_smplify_pkl(
        pkl_path: str,
        smpl_dir: str = "/root/development/project4010/VAE/models",
        vposer_dir: str | None = "/root/development/project4010/VAE/human_body_prior/support_data/downloads/vposer_v2_05",
        save_png: str | None = None):
    """
    SMPLify-X が出力した .pkl からメッシュ復元して quick_scatter で確認。

    * body_pose が 63 (axis-angle) ならそのまま使用
    * body_pose が 32 (VPoser latent) なら vposer_dir を使って decode
    ...
    pkl_path  : str
        変換したい .pkl ファイル
    smpl_dir  : str
        SMPL or SMPL-X の *.npz があるフォルダ
    vposer_dir: str | None
        VPoser ckpt フォルダ（32-dim のとき必須）
    save_png  : str | None
        画像保存先（None なら <pkl>.png）
    """
    import pickle, numpy as np, torch, trimesh
    from pathlib import Path
    from smplx.body_models import SMPL
    ...
    pose_raw = np.asarray(data['body_pose']).reshape(-1)
    if pose_raw.size == 63:
        body_pose_aa = torch.tensor(pose_raw, dtype=torch.float32, device=dev).unsqueeze(0)
    elif pose_raw.size == 32:
        if vposer_dir is None:
            raise ValueError("body_pose は 32 次元ですが vposer_dir=None です。ckpt パスを渡してください。")
        # ---- VPoser decode ----
        from human_body_prior.models.vposer_model import VPoser
        from human_body_prior.tools.model_loader import load_model
        vposer, _ = load_model(
        ...
        vposer = vposer.to(dev)
        with torch.no_grad():
            z = torch.tensor(pose_raw, dtype=torch.float32, device=dev).unsqueeze(0)
            # body_pose_aa = vposer.decode(z)['pose_body'].view(1, -1)
            body_pose_aa = vposer.decode(z)["pose_body"].reshape(z.size(0), -1)
            pose69 = torch.cat([body_pose_aa, torch.zeros(body_pose_aa.size(0), 6, device=body_pose_aa.device)], 1)
    else:
        raise ValueError(f"Unexpected body_pose dim = {pose_raw.size} (expect 32 or 63)")

    # ---------- 2. SMPL でメッシュ復元 ----------
    smpl = SMPL(model_path=smpl_dir, gender='NEUTRAL', batch_size=1).to(dev)
    out = smpl(
        betas         = torch.tensor(data.get('betas', np.zeros((1,10))), dtype=torch.float32, device=dev),
        global_orient = torch.zeros((1, 3), dtype=torch.float32, device=dev),
        # global_orient = torch.tensor(data['global_orient'], dtype=torch.float32, device=dev),
        body_pose     = pose69,
        transl        = torch.tensor(data.get('transl', np.zeros((1,3))), dtype=torch.float32, device=dev),
    )
    verts = out.vertices[0].cpu().numpy()

    import numpy as np, pathlib, torch
    verts_np = verts.detach().cpu().numpy() if torch.is_tensor(verts) else verts   # (N,3)

    # ---------- .xyz 形式で保存 ----------
    out_xyz = pathlib.Path(pkl_path).with_suffix('.xyz')
    np.savetxt(out_xyz, verts_np, fmt='%.6f')   # x y z 各列を空白区切り
    print('✓ saved →', out_xyz)

    # ---------- 3. 画像保存 ----------
    if save_png is None:
        save_png = Path(pkl_path).with_suffix('.png').name
    quick_scatter(save_png, verts, verts)   # 入力 verts が無いので左右同じ点群を描画
    print(f"✓ mesh preview saved → {save_png}")

    # （必要なら OBJ も保存）
    # trimesh.Trimesh(verts, smpl.faces, process=False).export(Path(pkl_path).with_suffix('.obj'))
...
if __name__ == "__main__":
    ...
    torch.manual_seed(0)
    # train(args)
    show_smplify_pkl('/mnt/c/Desktop/camera_output5/output/results/camera_0/000.pkl')
